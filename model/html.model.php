...
class htmlModel extends BaseModel
{
	...
	{	
		...
		$query = $this->handle->prepare("SELECT * FROM tbltest WHERE questionID = 1");
	...
	{	
		...
		$query = $this->handle->prepare("SELECT testID FROM tbltest WHERE questionID = 1");
		$query->execute();
	...
	{		
	    ...
	    $query=$this->handle->prepare("SELECT `testID`,`answer` FROM tbltest WHERE questionID = 1");
	    $query->execute(); 
