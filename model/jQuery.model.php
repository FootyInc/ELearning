<?php
class jQueryModel extends BaseModel
{
	...
	{
		$subject = '3';  	
		$query = $this->handle->prepare("INSERT INTO tbltest (questionID, creatorID, question, option1, option2, option3, option4, answer) VALUES (:qID, :creID, :testQuest, :testOp1, :testOp2, :testOp3, :testOp4, :testAns)");
	...
	{	
		...
		$query = $this->handle->prepare("SELECT * FROM tbltest WHERE questionID = 3");
	...
	{	
		...
		$query = $this->handle->prepare("SELECT testID FROM tbltest WHERE questionID = 3");
		$query->execute();
	...
	{		
	    ...
	    $query=$this->handle->prepare("SELECT `testID`,`answer` FROM tbltest WHERE questionID = 3");
	    $query->execute(); 
